...
public class Interface extends javax.swing.JFrame {
    ...
    private final Queue fila = new Queue(100); //permititrá guardar até 100 filmes
    private final List<Filmes> filmesCadastradosBackup = new ArrayList<>();
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        Filmes novoFilme = new Filmes(titulo, diretor, genero, pais, ano);
        filmesCadastradosBackup.add(novoFilme); //Adiciona na lista de Backup
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        if (!filmesCadastradosBackup.isEmpty()) {
            StringBuilder filmes = new StringBuilder();
            for(Filmes filme : filmesCadastradosBackup){
                filmes.append(filme.toString()).append("\n");
            }
            jTextArea1.setText(filmes.toString());
        } else {
            jTextArea1.setText("Nenhum filme cadastrado.");
        }
    }//GEN-LAST:event_jButton2ActionPerformed

    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        try{
            boolean FilmeAcao = false;
            jTextArea1.setText(""); // Limpa o conteúdo da área de texto
            while (!fila.isEmpty() && !FilmeAcao){
                Filmes filme = (Filmes) fila.peek();
                if(filme.getGenero().equalsIgnoreCase("ação")){
                    jTextArea1.append("Primeiro filme de ação encontrado:\n" + filme.toString() + "\n");
                    FilmeAcao = true;
                }else{
                    jTextArea1.append("Removendo filme não ação: " + filme.getTitulo() + "\n");
                    fila.dequeue();
                }
            }
            if(fila.isEmpty()){
                jTextArea1.append("Fila esvaziada. Não foi possivel encontrar filme de ação.\n");
            }
        } catch(Exception e){
            e.printStackTrace();
        }
